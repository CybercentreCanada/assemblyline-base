...
class Workflow(odm.Model):
    ...
    priority = odm.Enum(values=PRIORITIES, default_set=True)         # Priority applied by the workflow
    query = odm.Keyword()                                            # Query that the workflow runs
    status = odm.Enum(values=STATUSES, default_set=True)             # Status applied by the workflow
