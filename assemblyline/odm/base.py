...
class Model:
    ...
    def markdown(cls) -> str:
        markdown_content = "[comment]: # (AUTOGENERATED MARKDOWN CONTENT. DO NOT UPDATE MANUALLY.)\n"

        # Header
        markdown_content += f"### {cls.__name__} - {cls.description}\n"

        # Table
        table = "| Field | Type | Description | Required | Default |\n| :--- | :--- | :--- | :--- | :--- |\n"

        def get_type(field_class: _Field) -> str:
            if field_class.__class__ == Optional:
                return get_type(field_class.child_type)
            elif field_class.__class__ == Compound:
                return field_class.child_type.__name__
            elif field_class.__class__ in [Mapping, List]:
                child_type = field_class.child_type.__class__.__name__
                if field_class.child_type.__class__ == Compound:
                    child_type = get_type(field_class.child_type)
                return f"{field_class.__class__.__name__} [{child_type}]"
            elif field_class.__class__.__name__ == 'type':
                return field_class.__name__

            return field_class.__class__.__name__
        ...
        for field, info in cls.fields().items():
            ...
            row = f"| {field} | {field_type} | {description} | {required} | {info.default } |\n"
            table += row

        markdown_content += table + "\n\n"

        # Add descriptions about nested classes within the same model
