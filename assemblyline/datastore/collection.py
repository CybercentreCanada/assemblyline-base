...
class ESCollection(Generic[ModelType]):
    ...
    def __init__(self, datastore: ESStore, name, model_class=None, validate=True):
        ...
        self.shards = environ.get(
            f"ELASTIC_{name.upper()}_SHARDS", environ.get("ELASTIC_DEFAULT_SHARDS", 1)
        )
        self.archive_replicas = environ.get(
            f"ELASTIC_{name.upper()}_ARCHIVE_REPLICAS", self.replicas
        )
        self.archive_shards = environ.get(
            f"ELASTIC_{name.upper()}_ARCHIVE_SHARDS", self.shards
        )
        ...
        # If only archive asked
            ...
            if not self.archive_name:
                raise ArchiveDisabled(
                    f"Index {self.name.upper()} does not have an archive"
                )
    ...
    def scan_with_search_after(
        ...
        pit = {
            "id": self.with_retries(
                self.datastore.client.open_point_in_time,
                index=index,
                keep_alive=keep_alive,
            )["id"],
            "keep_alive": keep_alive,
    ...
    def _get_task_results(self, task):
        ...
        while res is None:
            try:
                ...
                msg = e.message
                if (
                    err_code == 500 or err_code == "500"
                ) and msg == "timeout_exception":
                    pass
                else:
                    raise
    ...
    def _get_current_alias(self, index: str) -> typing.Optional[str]:
        if self.with_retries(self.datastore.client.indices.exists_alias, name=index):
            return next(
                iter(
                    self.with_retries(
                        self.datastore.client.indices.get_alias, index=index
                    )
                ),
                None,
            )
    ...
    def _wait_for_status(self, index, min_status="yellow"):
        ...
        while not status_ok:
            try:
                res = self.datastore.client.cluster.health(
                    index=index, timeout="5s", wait_for_status=min_status
                )
                ...
                if err_code == 408 or err_code == "408":
                    log.warning(
                        f"Waiting for index {index} to get to status {min_status}..."
                    )
                    pass
                else:
                    raise
    ...
    def _safe_index_copy(
        self, copy_function, src, target, settings=None, min_status="yellow"
    ):
    ...
    def restore_old_archive(self, delete_after=False):
        ...
        source = {
            "index": f"{self.name}-0*",
            "query": {"bool": {"must": {"query_string": {"query": "id:*"}}}},
        }

        dest = {"index": f"{self.name}"}
        ...
        if delete_after:
            ...
            try:
                res = self.with_retries(
                    self.datastore.client.ilm.get_lifecycle, name=ilm_name
                )
                for index in (
                    res.get(ilm_name, {}).get("in_use_by", {}).get("indices", [])
                ):
                    self.with_retries(
                        self.datastore.client.ilm.remove_policy, index=index
                    )
                self.with_retries(
                    self.datastore.client.ilm.delete_lifecycle, name=ilm_name
                )
            ...
            if self.with_retries(
                self.datastore.client.indices.exists_template, name=self.name
            ):
                self.with_retries(
                    self.datastore.client.indices.delete_template, name=self.name
                )

            # Delete old archive alias
            alias = f"{self.name}-archive"
            if self.with_retries(
                self.datastore.client.indices.exists_alias, name=alias
            ):
                self.with_retries(
                    self.datastore.client.indices.delete_alias,
                    index=source["index"],
                    name=alias,
                )
            ...
            self.with_retries(
                self.datastore.client.indices.delete,
                index=source["index"],
    ...
    def bulk(self, operations):
        ...
        return self.with_retries(
            self.datastore.client.bulk, operations=operations.get_plan_data()
        )
    ...
    def fix_replicas(self, index_type=None):
        ...
        results = []
        for index in self.get_index_list(index_type):
            replicas = self._get_index_settings(archive=index == self.archive_name)[
                "index"
            ]["number_of_replicas"]
    ...
    def fix_shards(self, logger=None, index_type=None):
        ...
        for name in self.get_index_list(index_type):
            ...
            indexes_settings = self.with_retries(
                self.datastore.client.indices.get_settings, index=name
            )
            ...
            cur_replicas = int(
                current_settings["settings"]["index"]["number_of_replicas"]
            )
            ...
            if cur_shards > target_shards:
                ...
                if cur_shards % target_shards != 0:
                    logger.info(
                        "The target shards is not a factor of the current shards, aborting..."
                    )
                    return
                else:
                    target_node = self.with_retries(
                        self.datastore.client.cat.nodes, format="json"
                    )[0]["name"]
                ...
                if target_shards % cur_shards != 0:
                    logger.info(
                        "The current shards is not a factor of the target shards, aborting..."
                    )
                    return
                else:
            ...
            if method:
                ...
                if not self.with_retries(
                    self.datastore.client.indices.exists, index=temp_name
                ):
                    ...
                    if clone_setup_settings:
                        ...
                        while (
                            self.datastore.client.cluster.health(index=index)[
                                "relocating_shards"
                            ]
                            != 0
                        ):
                ...
                if self._get_current_alias(name) != temp_name:
                    logger.info(
                        f"Make {temp_name.upper()} the current alias for {name.upper()} "
                        f"and delete {index.upper()}."
                    )
                    # Make the hot index the temporary index while deleting the original index
                    actions = [
                        {"add": {"index": temp_name, "alias": name}},
                        {"remove_index": {"index": index}},
                    ]
                    self.with_retries(
                        self.datastore.client.indices.update_aliases, actions=actions
                    )
                ...
                logger.info(
                    f"Perform shard fix operation from {temp_name.upper()} to {index.upper()}."
                )
                self._safe_index_copy(method, temp_name, index, settings=settings)

                # Make the original index the new alias
                logger.info(
                    f"Make {index.upper()} the current alias for {name.upper()} "
    ...
    def reindex(self, index_type=None):
        ...
        for name in self.get_index_list(index_type):
            ...
            if self.with_retries(
                self.datastore.client.indices.exists, index=index
            ) and not self.with_retries(
                self.datastore.client.indices.exists, index=new_name
            ):
                # Get information about the index to reindex
                index_data = self.with_retries(
                    self.datastore.client.indices.get, index=index
                )[index]
                ...
                for alias, alias_data in index_data["aliases"].items():
                    ...
                    if alias_data.get("is_write_index", True):
                        ...
                        actions = [{"add": {"index": new_name, "alias": alias}}]
                    self.with_retries(
                        self.datastore.client.indices.update_aliases, actions=actions
                    )
                ...
                self.with_retries(
                    self.datastore.client.indices.clear_cache, index=new_name
                )
                ...
                try:
                    ...
                    if self.with_retries(
                        ...
                        self.with_retries(
                            self.datastore.client.indices.delete, index=new_name
                        )
                finally:
    ...
    def multiget(
        ...
        for index in index_list:
            ...
            data = self.with_retries(
                self.datastore.client.mget, ids=key_list, index=index
            )
    ...
    def normalize(self, data, as_obj=True) -> Union[ModelType, Dict[str, Any], None]:
        ...
        if (
            ...
            and not isinstance(data, self.model_class)
        ):
    ...
    def exists(self, key, index_type=None):
        ...
        for index in index_list:
            found = self.with_retries(
                self.datastore.client.exists, index=index, id=key, _source=False
            )
            if found:
                break
    ...
    def _get(self, key, retries, index_type=None, version=False):
        ...
        while not done:
            for index in index_list:
                try:
                    doc = self.with_retries(
                        self.datastore.client.get, index=index, id=key
                    )
                    ...
                    if version:
                        return (
                            self._normalize_output(doc["_source"]),
    ...
    def require(
        self, key, as_obj=True, index_type=None, version=False
        ...
        """
        data = self._get(
            key, self.RETRY_INFINITY, index_type=index_type, version=version
        )
        if version:
            data, version = data
            return self.normalize(data, as_obj=as_obj), version
    ...
    def save(self, key, data, version=None, index_type=Index.HOT):
        ...
        if " " in key:
            raise DataStoreException(
                "You are not allowed to use spaces in datastore keys."
            )
        ...
        for index in index_list:
            try:
                info = self.with_retries(
                    self.datastore.client.delete, id=key, index=index
                )
    ...
    def update(self, key, operations, index_type=Index.HOT):
        ...
        for index in index_list:
            try:
                res = self.with_retries(
                    self.datastore.client.update, index=index, id=key, script=script
                )
    ...
    def _search(
        ...
        for key, value in args:
            if key not in parsed_values:
                ...
                raise ValueError(
                    "Unknown query argument: %s %s of [%s]" % (key, value, all_args)
                )
        ...
        if parsed_values["group_active"]:
            query_body["collapse"] = {
                ...
                "inner_hits": {
                    "name": "group",
                    "_source": parsed_values["field_list"]
                    or list(self.stored_fields.keys()),
                    "size": parsed_values["group_limit"],
                    "sort": parse_sort(parsed_values["group_sort"])
                    or [{parsed_values["group_field"]: "asc"}],
                },
            }
        ...
        try:
            ...
            raise SearchRetryException(
                "collection: %s, query: %s, error: %s"
                % (self.name, query_body, str(error))
            )
    ...
    def search(
        ...
        ret_data = {
            ...
            "items": [
                self._format_output(doc, field_list, as_obj=as_obj)
        ...
        if pit_id is not None and len(ret_data["items"]) < ret_data["rows"]:
            ...
            search_after = base64.b64encode(
                json.dumps(result["hits"]["hits"][-1]["sort"]).encode()
            ).decode()
    ...
    def stream_search(
        ...
        if item_buffer_size > 2000 or item_buffer_size < 50:
            raise SearchException(
                "Variable item_buffer_size must be between 50 and 2000."
            )
        ...
        for value in self.scan_with_search_after(
    ...
    def keys(self, access_control=None, index_type=Index.HOT):
        ...
        """
        for item in self.stream_search(
            "id:*", fl="id", access_control=access_control, index_type=index_type
        ):
            try:
                yield item.id
        ...
        with warnings.catch_warnings():
            ...
            if not gaps_count:
                try:
                    ...
                    parsed_gap = (
                        dm(self.datastore.to_pydatemath(f"+{t_gap}")).int_timestamp
                        - dm("now").int_timestamp
                    )
            ...
            if gaps_count is None:
                raise SearchException(
                    ...
                    "have invalid date math values. (start='%s', end='%s', gap='%s')"
                    % (start, end, gap)
                )

            if gaps_count > self.MAX_HISTOGRAM_STEPS:
                raise SearchException(
                    f"Histograms are limited to a maximum of {self.MAX_HISTOGRAM_STEPS} steps. "
    ...
    def histogram(
        ...
        if filters is None:
            ...
            filters = [filters]
        filters.append(
            "{field}:[{min} TO {max}]".format(field=field, min=start, max=end)
        )
    ...
    def grouped_search(
        ...
        result = self._search(
            args, index_type=index_type, track_total_hits=track_total_hits
        )
            ...
            "items": [
                {
                    ...
                    "total": int(
                        collapsed["inner_hits"]["group"]["hits"]["total"]["value"]
                    ),
                    "items": [
    ...
    def _get_index_settings(self, archive=False) -> dict:
        ...
        settings["index"]["number_of_shards"] = (
            self.shards if not archive else self.archive_shards
        )
        settings["index"]["number_of_replicas"] = (
            self.replicas if not archive else self.archive_replicas
        )
        return settings
    ...
    def _get_index_mappings(self) -> dict:
        ...
        if self.model_class:
            mappings["properties"], mappings["dynamic_templates"] = build_mapping(
                self.model_class.fields().values()
            )
    ...
    def _check_fields(self, model=None):
        ...
        for field_name in matching:
            if (
                fields[field_name]["indexed"] != model[field_name].index
                and model[field_name].index
            ):
                raise RuntimeError(f"Field {field_name} should be indexed but is not.")

            possible_field_types = self.__get_possible_fields(
                model[field_name].__class__
            )
            ...
            if not self.with_retries(self.datastore.client.indices.exists, index=alias):
                ...
                try:
                    ...
                    if "resource_already_exists_exception" not in str(e):
                        raise
                    log.warning(
                        f"Tried to create an index template that already exists: {alias.upper()}"
                    )

                self.with_retries(
                    self.datastore.client.indices.put_alias, index=index, name=alias
                )
    ...
    def _add_fields(self, missing_fields: Dict):
        ...
        for name, field in missing_fields.items():
            ...
            sub_properties, sub_templates = build_mapping(
                [field], prefix=prefix, allow_refuse_implicit=False
            )
