...
def test_NamedConstants():
    NamedConstTest = str_utils.NamedConstants("test", [
        ("a", 1),
        ("B", 2),
        ("c", 3)
    ])
    # success tests
    assert NamedConstTest.name_for_value(1) == 'a'
    assert NamedConstTest.contains_value(1) is True
    assert NamedConstTest['a'] == 1
    assert NamedConstTest.c == 3
    # failure tests
    assert NamedConstTest.contains_value(4) is False
    with pytest.raises(KeyError):
        assert NamedConstTest.name_for_value(4) is None
        assert NamedConstTest['b'] == 2
        assert NamedConstTest.C == 3
...
def test_translate_str():
    ...
    encodedTestStr = teststr.encode('ISO-8859-5')
    result = str_utils.translate_str(encodedTestStr)
