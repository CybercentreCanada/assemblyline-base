...
class BaseTestModel(odm.Model):
    ...
    height = odm.Integer()
    no_store = odm.Keyword(store=False, default_set=True)
    no_index = odm.Keyword(index=False, store=False, default_set=True)
...
# noinspection PyShadowingNames
@pytest.mark.parametrize("function,as_obj", [(f[0], f[1]) for f in TEST_FUNCTIONS], ids=[f[2] for f in TEST_FUNCTIONS])
def test_riak(riak_connection, function, as_obj):
    function(riak_connection, as_obj)
...
def _test_c_streamsearch(s_tc, e_tc, r_tc, as_obj):
    ...
    ss_r_list = list(r_tc.stream_search('flavour:*', filters="height:[30 TO 400]", fl='flavour', as_obj=as_obj))
    assert compare_output(ss_s_list, ss_e_list, ss_r_list)


def _test_c_keys(s_tc, e_tc, r_tc, _):
    assert compare_output(sorted(list(s_tc.keys())), sorted(list(e_tc.keys())), sorted(list(r_tc.keys())))


def _test_c_histogram(s_tc, e_tc, r_tc, _):
...
def _test_c_groupsearch(s_tc, e_tc, r_tc, as_obj):
    assert compare_output(s_tc.grouped_search('height', fl='flavour', as_obj=as_obj),
                          ...
                          r_tc.grouped_search('height', fl='flavour', as_obj=as_obj))


def _test_c_fields(s_tc, e_tc, r_tc, _):
...
def test_datastore_consistency(riak_connection, solr_connection, es_connection, function, as_obj):
    if riak_connection and solr_connection and es_connection:
        function(solr_connection, es_connection, riak_connection, as_obj)
